...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Automatically redirect to homepage since we auto-login on successful registration
    ...
    <div className="min-h-[80vh] flex items-center justify-center bg-gray-50 px-4 py-20 mt-10 w-full">
       <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 max-w-md w-full">
          ...
          <form onSubmit={handleRegister} className="space-y-5">
             ...
             <button disabled={loading} type="submit" className="w-full bg-[#133A42] text-white font-semibold rounded-xl py-3 hover:bg-emerald-950 transition-colors disabled:opacity-50 mt-4">
                {loading ? 'Creating...' : 'Create Account'}
             </button>
             
             <div className="pt-6 text-center">
               <p className="text-sm text-gray-500 mb-2">
                  Already have an account? <Link href="/login" className="text-[#00A19D] font-bold hover:underline">Sign In</Link>
               </p>
             </div>
          </form>
       </div>
